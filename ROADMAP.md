...
| UI showcase (`rmlui_demo`) | 🟢 | Main menu, settings, inventory (drag & drop), HUD, dialogue/chat, loading — see README "The demo suite". Verified by screenshots + simulated clicks/drags/typing. |
...
| Asset browser / sandbox editor | 🟡 | `games/sandbox`: `kke::AssetCatalog` finds every pack on disk (any layout), filter by pack/category/search, ghost placement with grid snap + rotate + stacking, select/move/duplicate/delete, JSON layouts. Built on engine pieces: `kke::DebugDrawModule`, `kke/Picking.h`, `OrbitCameraModule::Controls::Editor`. **Not yet:** thumbnails, undo/redo, multi-select, gizmos, scale, collision for placed static meshes. |
| Destructible Synty props | 🟡 | Sandbox "Make breakable": the prop becomes a FEMFX box proxy of its bounds (wood/stone/glass break, metal dents). **Not yet:** shape-matched proxies coloured from the prop's texture, material-driven fracture patterns (task in progress), props resting on a broken one stay floating (static meshes have no collision). |
