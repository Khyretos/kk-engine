...
| 1.2 | **Character controller**: capsule, walk/run/jump/crouch, slopes, stairs, push objects, ride platforms | The "walk around" | 🔨 walk/slopes/stairs/jump/push/platforms done in `RigidWorld` (tested); crouch next |
| 1.3 | **Camera system**: first person, third-person spring arm (collision-aware, lag), orbit/editor, cinematic rails; **viewports** (split screen 1-4, picture-in-picture) | Asked for; split screen is also multiplayer groundwork | ⬜ |
| 1.4 | **Showcase level (kke_demo)**: Synty level + interaction stations: break props (FEMFX + Voronoi), pour lava/melt, water with floating things, ragdolls, lighting controls, RmlUi HUD/menus, ImGui debug | The demo that proves every system at once | ⬜ |
| 1.5 | **Animation basics** with the Synty animation library you sent: locomotion blend (idle/walk/run), state machine, root motion option, foot IK | A character that walks needs to *look* like it walks. Full animation demo is P2 (2.6) | ⬜ |
...
- **Physics, fluids, melting, buoyancy are CPU** today (FEMFX is CPU-only
  ...
  CPU fallback.
- **Not taking 100%**: 1.6 (resource governor) is the answer: frame cap,
  thread cap, render scale, and an explicit "use everything" switch.
