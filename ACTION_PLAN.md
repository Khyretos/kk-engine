...
|---|---|---|---|
| 1.1 | **Rigid-body + collision layer (Jolt Physics, MIT)** as a module next to FEMFX | Nothing static collided (placed Synty meshes were only visual; FEMFX knows a ground plane only). Also the scaling fix: 1,000 boxes = 1.56 ms/step on one thread | ✅ core (`kke::RigidWorld`, 7 tests); FEMFX<->Jolt bridge ✅ (`PhysicsBridgeModule`, docs/PHYSICS_BRIDGE.md, 5 tests, issue #7): Jolt bodies near moving FEMFX pieces become FEMFX boxes, measured contact pushes go back into Jolt, 0.05 ms a step; next (#31): rubble handoff, shared interface, Jolt ragdolls |
| 1.2 | **Character controller**: capsule, walk/run/jump/crouch, slopes, stairs, push objects, ride platforms | The "walk around" | 🔨 walk/slopes/stairs/jump/push/platforms done in `RigidWorld` (tested) and playable in kke_demo; crouch resizes the capsule (`setCharacterHeight`, refuses to stand under a ceiling; tested, and a low roof in kke_demo) ✅; `kke::Locomotion` (docs/MOVEMENT.md, 12 tests): vault/climb from dynamic area awareness, inertial turning, air control, floor tiers, coyote time + jump buffer; parkour lane in kke_demo ✅ |
| 1.3 | **Camera system**: first person, third-person spring arm (collision-aware, lag), orbit/editor, cinematic rails; **viewports** (split screen 1-4, picture-in-picture) | Asked for; split screen is also multiplayer groundwork | 🔨 `kke::CameraRig` (first/third person spring arm, orbit, cinematic; 4 tests) used by kke_demo (V toggles, wheel = arm length) ✅; viewports ✅: `Application::views()` draws 1-4 cameras into parts of the window (`kke/Viewports.h` split screen and picture-in-picture layouts, 4 tests; per-view lighting, culling and debug lines). kke_demo split screen for 2-4 players (a controller each, or they run the lane) and an overhead picture-in-picture (docs/INPUT.md "Split screen") |
| 1.4 | **Showcase level (kke_demo)**: Synty level + interaction stations: break props (FEMFX + Voronoi), pour lava/melt, water with floating things, ragdolls, lighting controls, RmlUi HUD/menus, ImGui debug | The demo that proves every system at once | 🔨 v1 playable: `games/showcase/` is now `kke_demo` (old one renamed `kke_basics`). Level with stairs, ramp, too-steep slope, pillars, moving platform, 18 Jolt crates (E/right click pushes), FEMFX breaking yard (glass, plank, stone wall; F/left click shoots), lighting + camera panels, F1 engine panels. Water station ✅: a walled pool (vault in, wade) where Jolt crates, planks and a raft float on small Gerstner waves (`kke::boxBuoyancy`, 5 tests; the same waves are drawn), steel sinks. FEMFX<->Jolt bridge ✅ (1.1): shards land on and push the crates under the yard's glass, an iron ball knocks a crate 0.84 m. Lava station ✅: a spout pours lava (`ParticleFluid`, 700-particle budget) into a stone basin onto a block that melts (`MeltVolume`: ice, wax, aluminium in turn); crates and characters push the lava aside; it only runs while a camera is within 18 m (about 2.7 ms a step when it runs). Next: RmlUi HUD, Synty level art |
| 1.5 | **Animation basics** with the Synty animation library you sent: locomotion blend (idle/walk/run), state machine, root motion option, foot IK | A character that walks needs to *look* like it walks. Full animation demo is P2 (2.6) | 🔨 `kke::Animator` (6 tests) drives the UAL mannequin in kke_demo: idle/walk/jog/sprint blend space synced to speed, crouch blend, jump start/loop/land, vault/climb stand-in poses (UAL Standard has no vault clips). Now also foot IK (hips drop on stairs/slopes), hand IK onto vault/climb edges, root motion as data, and any Synty SK_ character wearing the UAL clips (retargeted by bone name; kke_demo Character panel). `kke/AnimRig.h`, 10 tests. Feet tilt to match slopes. Real vault and climb clips from UAL volume 2 (SafetyVault, ClimbUp_1m/2m), posed by the move's progress with their lift removed (2 tests) |
| 1.6 | **Resource governor**: frame cap, physics/worker thread cap, render scale, "use everything" toggle, all in Settings | "Run with what it needs; take everything only if the user says so" | ✅ `kke::computeBudget` (ResourceGovernor.h): half the usable cores for physics workers (1..8), 144 fps cap with vsync off, 15 fps in the background, render scale 0.5..1 (3D drawn smaller and upscaled, UI stays sharp via the new `Module::renderOverlay`), "use everything" lifts it. `performance` section in settings.json; Performance panel in the showcase; `KKE_USE_EVERYTHING=1`. docs/OPTIMIZATION.md #26 |
| 1.7 | **Built-in stress test + benchmark log for end users** (like the build log: hardware + fps/frame times, one file) | Must always ship with the engine | ✅ In the showcase (`kke_demo`): Performance panel → "Run stress test", or `KKE_STRESS_TEST=1 ./kke_demo` (runs, writes, quits). 36 s fixed script, uncapped: walk (autopilot, animation + IK, shadows), a rain of 300 Jolt crates, impacts (FEMFX iron balls breaking the yard, or heavy Jolt blocks without FEMFX). One report `benchmark/stress_<time>_<host>.txt` (+ .json): hardware, settings, fps avg / 1% low / p50-p99 / worst frame, GPU and physics ms per phase, per-second samples and a verdict (smooth / playable / struggles / too slow). `kke::FrameStats`, tested. HW-015 |
| 1.8 | Natural fracture, seeds, debris budget, culling, instancing | Done this session (BUGS 045-051, OPTIMIZATION 21-25) | ✅ |
| 1.9 | **Input system** (asked for: every controller type, identical HOSAS sticks, Steam Controller gyro/paddles, input tester in RmlUi, controller menu navigation, Tarkov-style robust binding, left-handed) | Hardcoded keys block left-handed players; controllers are table stakes | ✅ `InputMap` + `InputDevices` + `InputModule` (17 tests), RmlUi Input screen, UI navigation, kke_demo on actions, left-handed mirror, virtual devices in CI. See docs/INPUT.md. Per-player controllers in kke_demo split screen (1.3). Next: gyro calibration/"ratchet" button, rumble on impacts |

## P2 — The systems that make it a platform

| # | Item | Notes | Status |
|---|---|---|---|
| 2.1 | **Networking module** | See "Networking" below: transport + rollback + voice, many peers per PC, dockerized server, lots of tests | 🔨 v1 ✅ (docs/NETWORKING.md): ENet transport behind `ITransport` (+ loopback and lag/jitter/loss simulation), bit-packed fuzz-tested protocol, authoritative host with owner-predicted players, server movement checks, snapshot interpolation, prioritised body snapshots, reliable events, LAN discovery, several hosts per PC; `NetModule` (Network panel, `KKE_NET=host`/`join:ADDRESS`) in kke_demo: players see each other walk and push the same crates. 19 tests. Next: input replay for competitive games (#28), dedicated server + Docker, voice, couch + online, replicated breakables |
...
| 2.4 | **Benchmark images + BENCHMARKS.md** | See "Hardware profiles" below | ✅ docs/BENCHMARKS.md: `kke_bench` (headless CPU suite, 8 seeded cases), Docker profiles floor to high + GPU passthrough + custom (`benchmarks/docker/`), `KKE_VRAM_BUDGET_MB` (VMA heap cap), stress report lists modules and VRAM cap, Benchmarks CI workflow tracks every push to main on the `benchmark-data` branch, community rows via the Benchmark result issue template |
| 2.5 | Cross-platform: Windows (MinGW in Docker: in progress; MSVC CI), Android (FEMFX SIMDe port), macOS/iOS (CI runners), Web (WebGPU backend) | docs/SCALING.md §D | 🔨 |
...
- **Vercidium** (read 2026-09-26, see docs/AUDIO.md): not FOSS (free for
...
- **Vercidium**: read, not FOSS (see above and docs/AUDIO.md).
