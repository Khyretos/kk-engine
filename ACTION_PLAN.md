...
|---|---|---|---|
| 1.1 | **Rigid-body + collision layer (Jolt Physics, MIT)** as a module next to FEMFX | Nothing static collided (placed Synty meshes were only visual; FEMFX knows a ground plane only). Also the scaling fix: 1,000 boxes = 1.56 ms/step on one thread | ✅ core (`kke::RigidWorld`, 7 tests); FEMFX<->Jolt bridge 🔨 (`PhysicsBridgeModule`, docs/PHYSICS_BRIDGE.md, 5 tests): Jolt bodies near moving FEMFX pieces become FEMFX boxes, measured contact pushes go back into Jolt; next: rubble handoff, shared interface |
