...
void ShowcaseModule::update(const kke::UpdateContext& ctx) {
    ...
    readActions(dt);
    // Input -> desired velocity, relative to the camera. An analog stick
    // gives partial speeds (walk by tilting a little).
    glm::vec3 move = m_rig.forward() * m_moveInput.y + m_rig.right() * m_moveInput.x;
    const float amount = std::min(1.0f, glm::length(m_moveInput));
    float speed = m_crouch ? kCrouchSpeed : m_sprint ? kSprintSpeed : m_walk ? kWalkSpeed : kJogSpeed;
    if (glm::length(move) > 1e-3f) move = glm::normalize(move) * speed * amount;
...
void ShowcaseModule::renderUi() {
    ...
    if (ImGui::CollapsingHeader("Controls", ImGuiTreeNodeFlags_DefaultOpen)) {
        ImGui::TextUnformatted("Keyboard: WASD move, Shift sprint, Alt walk, Space jump, C crouch,\nmouse look, wheel zoom, V view, left click shoot, E push, R reset,\nF1 engine panels, Esc frees the mouse.\n"
                               "Controller: left stick move, right stick look, A jump, B crouch,\nL3 sprint, RT shoot, Y push, R3 view, Start reset, Back panels.");
